...
# JSON Web Token, yang berarti token ini menggunakan JSON (
# Javascript Object Notation) berbentuk string panjang yang sangat random, 
# lalu token ini memungkinkan kita untuk mengirimkan data yang dapat 
# diverifikasi oleh dua pihak atau lebih.
import jwt
import datetime
# Dalam ilmu kriptografi, hash adalah algoritma yang dipakai untuk mengubah informasi. 
# Data yang dimasukkan nantinya diolah menjadi angka, huruf, atau karakter lain 
# menjadi karakter terenkripsi tanpa mengubah ukuran. 
# Data yang terenkripsi lewat fungsi hash tak bisa lagi Anda kembalikan karena satu arah
import hashlib
...
#Kunci untuk enkripsi data ketika mengirim dan menerima data dari client ke server 
SECRET_KEY = "YASMIN"
...
@app.route("/")
def home():
    #Menerima token dari Cookie 
    # adalah istilah untuk kumpulan informasi yang berisi 
    # rekam jejak dan aktivitas ketika menelusuri sebuah website
    token_receive = request.cookies.get("mytoken")
    try:
        #Mendeskripsi token, decode adalah proses sebaliknya yaitu konversi dari 
...
def user(username):
    # endopoint untuk menerima informasi profile user 
    # dan seluruh post mereka 
    token_receive = request.cookies.get("mytoken")
    try:
        payload = jwt.decode(token_receive, SECRET_KEY, algorithms=["HS256"])
        # jika ini profile saya pribadi, True
        # jika ini profile orang lain, False
